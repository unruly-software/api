...
import {
  type AllQueryKeysFor,
  type APIQueryConfig,
  defineAPI,
  type EndpointDefinition,
  type EndpointInput,
  type EndpointOutput,
  type PartialQueryKeysFor,
  type QueryKeyForEndpoint,
  type QueryKeysFor,
} from './endpoint';
...
describe('APIClient Type Inference', () => {
  ...
  describe('API Query Integration', () => {
    it('should support optional apiQuery configuration', () => {
      const api = defineAPI<{
        path: string;
        method: 'GET' | 'POST' | 'PUT' | 'DELETE';
      }>();

      const endpointWithoutApiQuery = api.defineEndpoint({
      ...
      const endpointWithApiQuery = api.defineEndpoint({
        request: z.object({ userId: z.number() }),
        response: z.object({
          id: z.number(),
          name: z.string(),
      ...
      expectTypeOf(endpointWithoutApiQuery.apiQuery).toMatchTypeOf<
        | APIQueryConfig<
            z.ZodObject<{ userId: z.ZodNumber }>,
            z.ZodObject<{
              id: z.ZodNumber;
              name: z.ZodString;
              email: z.ZodString;
      ...
      expectTypeOf(endpointWithApiQuery.apiQuery).toMatchTypeOf<
        | APIQueryConfig<
            z.ZodObject<{ userId: z.ZodNumber }>,
            z.ZodObject<{
              id: z.ZodNumber;
      ...
      // Verify the structure exists and is callable
      expect(endpointWithoutApiQuery.apiQuery).toBeUndefined();
      expect(endpointWithApiQuery.apiQuery).toBeDefined();
      expect(typeof endpointWithApiQuery.apiQuery?.queryKey).toBe('function');
    });

    it('should infer correct types for queryKey and invalidates functions', () => {
      const api = defineAPI<{ path: string; method: string }>();
        ...
        getUser: api.defineEndpoint({
          ...
          apiQuery: {
            queryKey: (request) => {
              ...
              >();
              return ['user', request?.userId] as const;
            },
          },
          metadata: { path: '/users/:userId', method: 'GET' },
        }),

        getTenants: api.defineEndpoint({
          request: z.object({ name: z.string().optional() }),
          response: z.array(z.object({ id: z.number(), name: z.string() })),
          apiQuery: {
            queryKey: (request) => ['tenants', request] as const,
          },
          metadata: { path: '/tenants', method: 'POST' },
        }),
      };

      expectTypeOf<QueryKeysFor<typeof userAPI>>().toEqualTypeOf<
        | ['user', number | undefined]
        | ['tenants', { name?: string | undefined } | undefined]
      >();

      expectTypeOf<PartialQueryKeysFor<typeof userAPI>>().toEqualTypeOf<
        | [a?: 'user', b?: number]
        | [a?: 'tenants', b?: { name?: string | undefined }]
      >();
    });

    it('should infer the default type of the query key for other methods', () => {
      const api = defineAPI<{ path: string; method: string }>();
      ...
      const userAPI = {
        ...
        healthcheck: api.defineEndpoint({
          request: z.object({ time: z.date() }),
          response: z.object({ status: z.string() }),
          metadata: { path: '/health', method: 'GET' },
        }),
        otherCheck: api.defineEndpoint({
          request: z.object({ time: z.string() }),
          response: z.object({ status: z.string() }),
          metadata: { path: '/other', method: 'GET' },
          apiQuery: {},
        }),
      };

      expectTypeOf<AllQueryKeysFor<typeof userAPI>>().toEqualTypeOf<
        | ['tenants', { name?: string | undefined } | undefined]
        | ['tenants']
        | ['healthcheck', { time: Date } | undefined]
        | ['healthcheck']
        | ['otherCheck', { time: string } | undefined]
        | ['otherCheck']
      >();
      ...
      expectTypeOf<PartialQueryKeysFor<typeof userAPI>>().toEqualTypeOf<
        | [a?: 'tenants', b?: { name?: string | undefined } | undefined]
        | [a?: 'healthcheck', b?: { time: Date } | undefined]
        | [a?: 'otherCheck', b?: { time: string } | undefined]
      >();

      expectTypeOf<
        QueryKeyForEndpoint<'healthcheck', typeof userAPI.healthcheck>
      >().toEqualTypeOf<['healthcheck', { time: Date } | undefined]>();
    });
  });
});
